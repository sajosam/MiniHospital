...
def availability(request):
    ...
    if request.method == 'POST':
        ...
        #
        av = availabilitycheck.availablility_check(
            id=doc_id, date=date, user_id=request.user.id)
        lst1 = av.check()
        if lst1 is not None:
            obj = lst1
        else:
...
def viewpatientappo(request):
    lst = appointmentconfirmation.objects.filter(
        user_id=request.user.id).order_by('-appo_date')
...
def reschedule(request, id=None):
    l = list(appointmentconfirmation.objects.filter(
        id=id).values_list('appo_date'))
...
def patalldetailsupdate(request):  # imp
    ...
    if request.method == 'GET':
        ...
        context['usr1'] = UserForm(
            instance=Account.objects.get(id=request.user.id))
        ...
        if usr.is_valid() and usr1.is_valid():
            ...
            usr.user_id = Account.objects.only(
                'id').get(email=request.user.email)
            usr.save()
...
def appointment(request, id=None, time=None):
    ...
    client = razorpay.Client(
        auth=("rzp_test_19au902WXB3fFT", "oXzTgMla8l4NduIz8hIWkQLp"))
...
def confirmappointment(request):
    ...
    if 'payonline' in urls:
        data = appointmentconfirmation.objects.get(
            id=request.session['appo_id'])
        data.payment = 'paid'
        data.payment_id = Payment.objects.only('id').get(
            razorpay_order_id=request.session['order_id'])
        data.save()
        payment = Payment.objects.get(
            razorpay_order_id=request.session['order_id'])
        payment.appo_id = appointmentconfirmation.objects.only(
            'id').get(id=request.session['appo_id'])
        payment.save()

    else:
        usr = appointmentconfirmation()
        usr.user_id = Account.objects.only('id').get(id=request.user.id)
        usr.doc_email = Account.objects.only('id').get(
        ...
        if payment:
            usr.payment = 'paid'
            usr.payment_id = Payment.objects.only('id').get(
                razorpay_order_id=request.session['order_id'])
        else:
            usr.payment = 'unpaid'
        usr.save()
        if payment:
            payment = Payment.objects.get(
                razorpay_order_id=request.session['order_id'])
            payment.appo_id = appointmentconfirmation.objects.only('id').get(user_id=Account.objects.only('id').get(id=request.user.id), doc_email=Account.objects.only(
                'id').get(email=request.session['doc_email']), appo_date=request.session['date'], appo_time=request.session['time'])
            payment.save()
        ...
        message = client.messages.create(
            ...
            body='Your appointment is confirmed with '+dc.first_name+dc.last_name +
            ' on '+request.session['date']+' at '+request.session['time'],
            to='+918139835592'
        )
        message = client.messages.create(
